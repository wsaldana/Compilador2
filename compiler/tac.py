...
class SymbolsVisitor(yaplVisitor):
    ...
    def get_t(self, current=False):
        if not current:
            self.t_count += 1
        return f'_t{self.t_count}'

    def get_l(self):
        self.l_count += 1
        return f'_L{self.l_count}'

    def visitSimple_var(self, ctx: yaplParser.Simple_varContext):
        val = ctx.getText()
        if isinstance(
            ctx.parentCtx,
            (yaplParser.Statement_braceContext, yaplParser.MethodDefContext)
        ):
            if ctx.STR_VAR():
                print(f'RETURN "{val}"')
            else:
    ...
    def visitParams(self, ctx: yaplParser.ParamsContext):
        self.returns.append(len(self.returns))
        args = ctx.expr()
        if not isinstance(args, list):
            args = [args]
        ...
        name = ctx.ID_VAR()
        print(f"CALL {name}")
        r = "NONE" if not len(self.returns) else self.returns.pop()
        print(f"{res} = GET {r}")
    ...
    def visitMethodDef(self, ctx: yaplParser.MethodDefContext):
        ...
        for child in ctx.children:
            if isinstance(child, yaplParser.Id_varContext):
                print(f'RETURN {child.getText()}')
        print("END_FUNC\n")
